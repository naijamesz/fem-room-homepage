    ...
    <>
      <header
        ...
        <nav
          ...
          <div className='z-10 rounded-full hamburger md:hidden'>
            <Ripples>
              <button onClick={() => setIsactive(false)} className='rounded-full w-9 aspect-square'>
                <img className='mx-auto' src={CloseHamburger} alt='Close Hamburger Menu Image' />
              </button>
            </Ripples>
          </div>
        ...
        <button
          ...
          className={`w-9 aspect-square md:hidden rounded-full transition-all duration-500 ease-in-out ${
            isactive ? 'opacity-0 translate-y-4 pointer-events-none' : 'opacity-100 translate-y-0 delay-[-75ms]'
          }`}>
